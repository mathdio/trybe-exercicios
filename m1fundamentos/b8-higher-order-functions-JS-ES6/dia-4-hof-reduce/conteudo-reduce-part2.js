...
const highestValue = numbers.reduce((acc, curr) => acc > curr ? acc : curr);

// const highestValue = numbers.reduce((acc, curr) => {
...
const bestSubject = (acc, curr) => {
  return acc.nota > curr.nota ? acc : curr;
}

const report = (array) => array.map((estudante) => ({
  name: estudante.nome,
  materia: estudante.materias.reduce(bestSubject).name,
}));
